/**
 * LangSmith Evaluation Runner for Wigway
 * 
 * Usage:
...
// ============================================================================
// Target function - wraps Wigway agent
// ============================================================================
...
function createEvaluationRunner(sampleSize?: number) {
  return async function* runEvaluation(): AsyncGenerator<EvalProgressEvent, void, unknown> {
    ...
    const datasetName = sampleSize
      ? `master-wigway-finance-eval-sample-${sampleSize}-${Date.now()}`
      : 'master-wigway-finance-eval';
    ...
    const experimentName = `master-wigway-eval-${Date.now().toString(36)}`;
